 ...
 *
 *  1. grammatik against Chevrotain, a mature parser toolkit whose rule bodies
 *     are ordinary JavaScript that V8 JITs directly. A demanding baseline.
 *  2. grammatik against itself with compilation, LL(1) dispatch and
...
report('JSON, small document (lex + parse + build value)', runAll([
  ...
  ['grammatik (compiled)', 'small:compiled'],
  ['grammatik (interpreted)', 'small:interpreted'],
  ['grammatik (compiled + memo)', 'small:memo'],
  ['grammatik (backtracking)', 'small:backtracking']
]), 'grammatik (compiled)')

report('JSON, large document (lex + parse + build value)', runAll([
  ['JSON.parse (native)', 'large:native'],
  ['chevrotain', 'large:chevrotain'],
  ['grammatik (compiled)', 'large:compiled'],
  ['grammatik (interpreted)', 'large:interpreted'],
  ['grammatik (compiled + memo)', 'large:memo'],
  ['grammatik (backtracking)', 'large:backtracking'],
  ['grammatik (compiled, offsets only)', 'large:offsets']
]), 'grammatik (compiled)')
...
report('JSON, large document — lexing only', runAll([
  ...
  ['grammatik lexer (full line+col)', 'lex:jl'],
  ['grammatik lexer (offsets only)', 'lex:jl-offset']
]), 'grammatik lexer (full line+col)')

report('JSON, large document — parsing only (pre-lexed)', runAll([
  ['chevrotain parser', 'parse:chevrotain'],
  ['grammatik parser (compiled)', 'parse:compiled'],
  ['grammatik parser (interpreted)', 'parse:interpreted']
]), 'grammatik parser (compiled)')

report('Exponential backtracking, depth 15 (2^15 leaf visits unmemoized)', runAll([
  ['grammatik (no memo)', 'exp:nomemo'],
  ['grammatik (memo)', 'exp:memo']
]), 'grammatik (no memo)')

report('Arithmetic expressions (action on every node)', runAll([
  ['grammatik (compiled)', 'calc:compiled'],
  ['grammatik (interpreted)', 'calc:interpreted'],
  ['grammatik (compiled + memo)', 'calc:memo']
]), 'grammatik (compiled)')
