 ...
 *
 * The file imports `jl-grammar/runtime` for the lexer and parse state, and
 ...
 *           validate?: boolean, moduleName?: string, runtimeSpecifier?: string,
 *           engineSpecifier?: string, methods?: Record<string, any> }} [options]
...
export function emitModule (spec, options = {}) {
  ...
  const {
    ...
    runtimeSpecifier = 'jl-grammar/runtime',
  ...
  for (const [name, method] of Object.entries(semanticMethods || {})) compileEngine.addMethod(name, method)
  ...
  return `// Generated by jl-grammar from the ${quote(moduleName)} grammar. Do not edit.
//
...
${engineFree ? '' : `import { LogicEngine } from ${quote(engineSpecifier)}\n`}

export const tokens = ${literal(tokenDefs)}

export const start = ${quote(start)}

const lexer = createLexer(tokens, { positions: ${quote(positions)} })
export const tokenize = lexer.tokenize
 ...
 * @param {string} [source]
 */
export function parseTokens (tokenList, source, track = false) {
  ...
  const trailing = ${strict} && state.idx < tokenList.length
...
export default { parse, parseTokens, tokenize, tokens, start, registerMethods }
`
}
